...
class TaskState:
    ...
    steps_made: int
    """The total number of distinct steps that have been initiated since the beginning of the task. This increments regardless of the step's outcome."""

    consecutive_failed_steps: int
    """An integer counter for the number of consecutive steps that have received a `StepVerdict.FAILURE` from the judge. This counter resets to 0 upon a `StepVerdict.SUCCESS` or `StepVerdict.PARTIAL`."""
...
class StepState(TaskState):
    ...
    attempt: int
    """The current attempt number within the ongoing step. This increments with each new attempt to complete the current step."""
    consecutive_failed_attempts: int
    """The number of consecutive attempts within the current step that have resulted in a non-successful outcome (i.e., `StepVerdict.FAILURE` or an inability to generate a step summary/evaluation). This counter resets to 0 upon a successful attempt."""
    feedback: Optional[str]
    """The feedback provided by the judge from the immediately preceding attempt. This feedback guides subsequent attempts within the same step."""
...
class Evaluate(StepState):
    ...
    step_summary: str
    """A natural-language summary of the work performed in the current attempt, generated by the LLM. This summary is used by the judge to evaluate the step."""
...
class Complete:
    ...
    """A descriptive message indicating the reason for the task step's completion, often containing the final evaluation feedback."""
    attempt: int = 0
    """The attempt number at which the task step was successfully completed."""
...
class Failed:
    ...
    status: Optional[str] = None
    """A descriptive message indicating the reason for the task step's failure."""
    attempt: int = 0
    """The attempt number at which the task step failed."""
...
class StepPhaseResult:
    ...
    """The overall status of the step phase, indicating whether it completed successfully, failed, or needs to continue."""
    feedback: Optional[str] = None
    """Additional information or feedback related to the step phase's outcome."""
    attempt: int = 0
    """The attempt number at which the step phase concluded."""
...
class ImplementationPhaseResult:
    ...
    """The final status of the entire implementation phase: 'complete' if the task was finished, 'failed' if an unrecoverable error occurred, or 'interrupted' if stopped by the user."""
    feedback: Optional[str] = None
    """A message providing more details about the outcome of the implementation phase."""
    attempt: int = 0
    """The attempt number at which the implementation phase concluded."""
